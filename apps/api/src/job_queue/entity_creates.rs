//! Persisted `background_jobs` workers for ticket / todo / time entry / link / collection mutations
//! (creates, updates, deletes, ticket comments, time breaks, and bulk time ops).
...
async fn exec_time_entry_timer_create(
    ...
    let billing = match crate::time_entry_billing::resolve_time_entry_billing(
        pool,
        &user_id,
    ...
    {
        Ok(b) => b,
        Err(ae) => return JobExecOutcome::Fail(ae),
    };
