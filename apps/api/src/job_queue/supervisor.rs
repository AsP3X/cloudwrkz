...
impl JobWorkerSupervisor {
    ...
    pub fn job_runs(&self) -> Arc<JobRunRegistry> {
        self.job_runs.clone()
    }
...
pub fn spawn_job_queue_supervisor(
    ...
    let job_runs = Arc::new(JobRunRegistry::new());
...
async fn supervisor_loop(
    ...
    super::reclaim_stale_processing_jobs(&pool, &budgets).await;
    ...
    loop {
        ...
        while running.len() < want {
            ...
            let job_runs_task = job_runs.clone();
