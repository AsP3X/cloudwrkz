...
pub(super) async fn exec_time_entry_update(
    ...
    {
        if (body.customer_id.is_some() || body.customer_contact_id.is_some())
            && !crate::time_entry_billing::customers_module_enabled(pool).await
        {
            let _ = tx.rollback().await;
            return JobExecOutcome::Fail(AppError::bad_request(
        ...
        let customer_contact_id = match body.customer_contact_id.as_ref() {
            None => {
                if body.customer_id.as_ref().is_some_and(|v| v.is_null()) {
                    None
                } else {
                    existing.customer_contact_id.clone()
                }
            }
            Some(value) if value.is_null() => None,
            Some(value) => value.as_str().map(|s| s.to_string()),
        ...
        if let (Some(cid), Some(ccid)) = (&customer_id, &customer_contact_id) {
            ...
            if ok.is_none() {
                let _ = tx.rollback().await;
                return JobExecOutcome::Fail(AppError::not_found("Customer contact not found"));
            }
        }
        ...
        if let Err(e) = sqlx::query(
            r#"UPDATE time_entries SET customer_id = $1, customer_contact_id = $2,
               hourly_rate = $3, updated_at = NOW() WHERE id = $4"#,
        )
        .bind(&customer_id)
