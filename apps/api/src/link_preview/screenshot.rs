...
// Human: When robots.txt allows fetching, we capture a PNG of the page with headless Chromium and serve it from `/api/screenshots/`.
// Agent: SPAWNS chromium --headless --screenshot; WRITES public/uploads/link-screenshots/screenshot-{linkId}.png; RETURNS /screenshots/... path for v1 base URL.
...
const UPLOAD_DIR: &str = "public/uploads/link-screenshots";
const DEFAULT_WIDTH: u32 = 1280;
const DEFAULT_HEIGHT: u32 = 720;
const DEFAULT_TIMEOUT_SECS: u64 = 45;

// Human: Screenshot capture is optional in dev when Chromium is not installed; production Docker images include it.
...
// Agent: REQUIRES url_safe_for_outbound_fetch; SPAWNS chromium; WRITES UPLOAD_DIR/screenshot-{id}.png; RETURNS /screenshots/... or None.
...
pub async fn capture_link_screenshot(page_url: &str, link_id: &str) -> Option<String> {
    ...
    let disk_path = Path::new(UPLOAD_DIR).join(&filename);
