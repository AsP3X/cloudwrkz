...
export function AddLinkDialog({ open, onOpenChange, selectedCollectionId, selectedCollectionName, selectedCollectionColor, onSuccess }: AddLinkDialogProps) {
  type DuplicateLinkSummary = { id: string; title: string; url: string; created_at: string };
  ...
  const [duplicateLinks, setDuplicateLinks] = React.useState<DuplicateLinkSummary[]>([]);
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(() => {
      api.get<string[]>(`/links/tag-suggestions?q=${encodeURIComponent(q)}`)
        .then((suggestions) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await api.post<{
        ...
        id?: string;
        error?: string;
        duplicate_link_ids?: string[];
        similar_link_ids?: string[];
      }>("/links", {
        url: formattedUrl,
        ...
        collection_ids: finalCollectionId ? [finalCollectionId] : [],
      ...
      if (result.id) {
        onOpenChange(false);
        onSuccess?.();
      } else if (result.duplicate_link_ids?.length || result.similar_link_ids?.length) {
        setHasExactDuplicate(!!(result.duplicate_link_ids?.length));
        const ids = result.duplicate_link_ids || result.similar_link_ids || [];
        if (ids.length > 0) {
          try {
  ...
  const handleProceedWithDuplicate = async () => {
    ...
    try {
      const finalCollectionId = manuallySelectedCollectionId || (!collectionRemoved && selectedCollectionId ? selectedCollectionId : null);
      const result = await api.post<{ id?: string; error?: string }>("/links", {
        url: formattedUrl,
        title: title.trim() || undefined,
        description: description.trim() || undefined,
        tags,
  ...
  return (
    <>
      <Dialog
        ...
        className={cn(
          "max-w-2xl transition-transform transition-opacity duration-150",
          showDuplicateDialog && "scale-[0.98] opacity-60 pointer-events-none"
        )}
      >
        <form onSubmit={handleSubmit} className="px-4 sm:px-6 py-4 sm:py-6">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label htmlFor="add-link-tags" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">
                  Tags
                </label>
      ...
      {showDuplicateDialog && (
        ...
        >
          <div className="p-6 space-y-6 bg-white dark:bg-neutral-900 rounded-b-xl shadow-2xl">
            <div className="space-y-4">
              ...
              {duplicateLinks.length > 0 && (
                <div className="space-y-2">
                  <p className="text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wide">
                    Existing link{duplicateLinks.length > 1 ? "s" : ""} with this URL
                  </p>
                  <div className="space-y-2">
                    {duplicateLinks.map((item) => (
                      <div
                        key={item.id}
                        className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 rounded-md border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-3 py-2"
                      >
                        <div className="min-w-0">
                          <Link
                            to={`/dashboard/links/${item.id}`}
                            className="text-sm font-medium text-primary-700 dark:text-primary-300 hover:text-primary-900 dark:hover:text-primary-100 truncate"
                          >
                            {item.title}
                          </Link>
                          <a
                        ...
                        <div className="flex flex-shrink-0 gap-2">
                          <Link
                            to={`/dashboard/links/${item.id}`}
                            className="inline-flex items-center px-2 py-1 text-xs font-medium border border-neutral-300 dark:border-neutral-600 text-neutral-700 dark:text-neutral-300 hover:bg-neutral-50 dark:hover:bg-neutral-800 rounded-md"
                          >
                            View details
                          </Link>
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center px-2 py-1 text-xs font-medium text-primary-600 dark:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-md"
                          >
                            Open URL
                          </a>
                        </div>
