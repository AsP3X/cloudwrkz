...
export const useLinkDetailSidebar = () => React.useContext(LinkDetailSidebarContext);

export function LinkDetailSidebarProvider({
  defaultOpen = true,
  children,
}: {
  defaultOpen?: boolean;
  ...
  return (
    <LinkDetailSidebarContext.Provider value={{ isOpen, setIsOpen }}>{children}</LinkDetailSidebarContext.Provider>
  );
}
...
export const LinkDetailLayout = ({ sidebar, children }: LinkDetailLayoutProps) => {
  const { isOpen: desktopSidebarOpen, setIsOpen: setDesktopSidebarOpen } = useLinkDetailSidebar();
  ...
  return (
    <div className="relative">
      ...
      <div
        className={cn("lg:hidden fixed inset-0 z-50", mobileDrawerOpen ? "" : "pointer-events-none")}
      ...
      >
        ...
        >
          <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-neutral-200 dark:border-neutral-800">
            <div className="min-w-0">
              <p className="text-sm font-semibold text-neutral-900 dark:text-neutral-100 truncate">Link information</p>
            </div>
            <Button type="button" variant="outline" size="sm" onClick={() => setMobileDrawerOpen(false)}>
              Close
            </Button>
          </div>
