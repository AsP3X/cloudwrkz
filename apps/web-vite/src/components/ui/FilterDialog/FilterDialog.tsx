...
const FilterDialogInner = ({
  ...
  React.useEffect(() => {
    ...
    const loadPresets = async () => {
      try {
        ...
        const backendResult = await api.get<{
          presets: FilterPreset[];
          lastUsedPresetId: string;
        }>(`/filter-preferences/${config.moduleName}/presets`);

        const backendPresets = backendResult?.presets || [];
        const backendLastUsed = backendResult?.lastUsedPresetId || "";
        ...
        try {
          ...
          console.error("Failed to load filter presets from localStorage:", localError);
          setSavedPresets([]);
          setLastUsedPresetId("");
        }
      }
  ...
  const applyFilters = async () => {
    ...
    try {
      if (Object.keys(filtersToSave).length > 0) {
        await api.post(`/filter-preferences/${config.moduleName}`, { filters: filtersToSave });
      } else {
        await api.delete(`/filter-preferences/${config.moduleName}`);
      }
    } catch (error) {
      // Don't block on failure - localStorage is optional
    }

    navigate(`${config.baseRoute}?${params.toString()}`);
    onOpenChange(false);
  };
  ...
  const savePreset = async () => {
    ...
    try {
      await api.post(`/filter-preferences/${config.moduleName}/presets`, newPreset);
    } catch (error) {
      // API may not exist - fall back to localStorage only
    }
    try {
  ...
  const loadPreset = async (presetId: string) => {
    ...
    if (preset) {
      ...
      try {
        await api.post(`/filter-preferences/${config.moduleName}/last-used`, { presetId });
        try {
  ...
  const clearPreset = async () => {
    ...
    try {
      await api.post(`/filter-preferences/${config.moduleName}/last-used`, { presetId: null });
      try {
        localStorage.removeItem(lastUsedKey);
      } catch (localError) {
        // Ignore localStorage errors
    ...
    navigate(config.baseRoute, { replace: true });
    onOpenChange(false);
  };
  ...
  const deletePreset = async (presetId: string) => {
    ...
    try {
      await api.delete(`/filter-preferences/${config.moduleName}/presets/${presetId}`);
    } catch (error) {
      // API may not exist - fall back to localStorage only
    }
    try {
      ...
      <div className="px-4 py-4 md:px-6 md:py-6 space-y-4 md:space-y-6 min-w-0">
        ...
        {enablePresets && (
          <div className="border-t border-neutral-200 dark:border-neutral-700 pt-3 md:pt-4 min-w-0">
            <label htmlFor="filter-preset-name" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">
