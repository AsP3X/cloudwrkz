...
- **Authentication & Accounts**: Email/password login, registration, account deletion, session management, and banned-user handling.
- **Dashboard**: Unified dashboard with navigation, profile page, and global search across tickets, todos, links, and time entries.
- **Tickets**: Support tickets with rich text, comments, attachments, status/priority, and per-ticket time tracking.
- **Todos**: Lightweight task/todo items (including ticket subtasks) with assignment, status, and due dates.
- **Time Tracking**: Timers and manual entries, ticket-linked time, basic statistics, and CLI tools for managing entries.
- **Links & Collections**: Personal and shared link collections with metadata extraction, archive views, filters, and pagination.
- **Admin & Permissions**: Admin dashboard for users, groups, sessions, modules, and a group-based permission system with fine-grained rights.
- **CLI Tooling**: Full-featured interactive CLI (`pnpm cli`) for managing users, groups, modules, sessions, todos, tickets, time tracking, permissions, statistics, and DB utilities.
