...
Command-line interface for managing CloudWrkz users, groups, modules, tickets, todos, time tracking, permissions, sessions, statistics, and database maintenance.
...
Or start the **interactive management console**:
...
**Module keys:** `tickets`, `timetracking`, `todos`, `links`
...
The standalone Projects module and its CLI commands have been removed from the application in favor of tickets, todos, and time tracking.  
See `PROJECTS_REMOVAL_PLAN.md` for the full migration and cleanup plan.
