...
export function ArchivePageClient({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Type tabs */}
      {(() => {
        // Compute per-type counts from ALL items (ignoring query/date filters) for accurate tab badges
        const allTypeCounts = {
        ...
        type Tab = { key: ArchiveItemType; label: string; count: number; show: boolean };
        const tabs: Tab[] = ([
          { key: "all" as ArchiveItemType, label: "All", count: totalCount, show: true },
          { key: "tickets" as ArchiveItemType, label: "Tickets", count: allTypeCounts.tickets, show: canView.tickets },
          { key: "todos" as ArchiveItemType, label: "Todos", count: allTypeCounts.todos, show: canView.todos },
          { key: "time" as ArchiveItemType, label: "Time Entries", count: allTypeCounts.time, show: canView.time },
          { key: "links" as ArchiveItemType, label: "Links", count: allTypeCounts.links, show: canView.links },
        ] as Tab[]).filter((t) => t.show);
