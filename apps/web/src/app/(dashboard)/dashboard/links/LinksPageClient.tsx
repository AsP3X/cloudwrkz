...
import { ExportLinksDialog } from "@/components/features/links/ExportLinksDialog";
...
const EMPTY_COLLECTIONS: NonNullable<LinksPageProviderProps['collections']> = [];

export function LinksPageProvider(props: LinksPageProviderProps) {
  return (
    <Suspense fallback={null}>
      <LinksPageProviderInner {...props} />
    </Suspense>
  );
}

function LinksPageProviderInner({ children, canCreate, collections = EMPTY_COLLECTIONS }: LinksPageProviderProps) {
  ...
  return (
    <LinksPageContext.Provider value={{ openAddLink, openBulkAdd, openImport, openExport, openCreateCollection }}>
      {children}
      <ExportLinksDialog
        open={exportOpen}
        onOpenChange={setExportOpen}
        filters={exportFilters}
        contextLabel={selectedCollection ? `“${selectedCollection.name}” collection` : undefined}
      />
      {canCreate && (
        <>
          <AddLinkDialog 
...
export function LinksOverviewActionsMenu({ canCreate, collectionId, collections = EMPTY_COLLECTIONS }: LinksOverviewActionsMenuProps) {
  const { openImport, openExport } = useLinksPage();
