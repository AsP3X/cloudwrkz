 ...
 *   pnpm cli permission grant <group> <permission|all>
 *   pnpm cli permission revoke <group> <permission|all>
 *   pnpm cli permission list-group <group>
 *   pnpm cli permission grant-user <user-email> <permission|all>
 *   pnpm cli permission revoke-user <user-email> <permission|all>
...
Commands:
  ...
  grant <group> <permission|all>                 Grant permission(s) to group (use "all" for all)
  revoke <group> <permission|all>                 Revoke permission(s) from group (use "all" for all)
  list-group <group>                             List permissions for a group
  grant-user <user-email> <permission|all>       Grant permission(s) to user (use "all" for all)
  revoke-user <user-email> <permission|all>     Revoke permission(s) from user (use "all" for all)
...
async function handleGrant(groupNameArg?: string, permissionKeyArg?: string) {
  ...
  if (!groupName || !permissionKey) {
    console.error("Usage: grant <group> <permission|all>");
    process.exit(1);
  }

  const isAll = permissionKey.toLowerCase() === "all";
  const spinner = createSpinner(isAll ? "Granting all permissions..." : "Granting permission...");
  spinner.start();
  ...
  try {
    ...
    if (isAll) {
      ...
      if (allPermissions.length === 0) {
        spinner.succeed("No permissions in database");
        notice("Run pnpm db:seed-permissions to seed permissions first.", "info");
        return;
      }
      const result = await prisma.groupPermission.createMany({
        data: allPermissions.map((p) => ({ groupId: group.id, permissionId: p.id })),
        skipDuplicates: true,
      });
      spinner.succeed("Permissions granted");
      console.log(`\n✅ ${result.count} permission(s) granted to group "${groupName}" (${allPermissions.length - result.count} already had).`);
      return;
    }
...
async function handleRevoke(groupNameArg?: string, permissionKeyArg?: string) {
  ...
  if (!groupName || !permissionKey) {
    console.error("Usage: revoke <group> <permission|all>");
    process.exit(1);
  }

  const isAll = permissionKey.toLowerCase() === "all";
  const spinner = createSpinner(isAll ? "Revoking all permissions..." : "Revoking permission...");
  spinner.start();
  ...
  try {
    ...
    if (isAll) {
      const result = await prisma.groupPermission.deleteMany({
        where: { groupId: group.id },
      });
      spinner.succeed("Permissions revoked");
      console.log(`\n✅ ${result.count} permission(s) revoked from group "${groupName}".`);
      return;
    }
...
async function handleGrantUser(userEmailArg?: string, permissionKeyArg?: string) {
  ...
  if (!userEmail || !permissionKey) {
    const errorMsg = "Usage: grant-user <user-email> <permission|all>";
  ...
  try {
    ...
    if (isAll) {
      ...
      const result = await prisma.userPermission.createMany({
        data: allPermissions.map((p) => ({ userId: user.id, permissionId: p.id })),
        skipDuplicates: true,
      });
      spinner.succeed("Permissions granted");
      console.log(`\n✅ ${result.count} permission(s) granted to user "${userEmail}" (${allPermissions.length - result.count} already had).`);
      return;
    }
...
async function handleRevokeUser(userEmailArg?: string, permissionKeyArg?: string) {
  ...
  if (!userEmail || !permissionKey) {
    const errorMsg = "Usage: revoke-user <user-email> <permission|all>";
    console.error(errorMsg);
    if (!userEmailArg) {
      // Called from command line, exit
      process.exit(1);
  ...
  try {
    ...
    if (isAll) {
      ...
      spinner.succeed("Permissions revoked");
      console.log(`\n✅ ${result.count} permission(s) revoked from user "${userEmail}".`);
      return;
    }
...
export async function handleGrantInteractive() {
  try {
    ...
    const grantAll = await confirm("Grant all permissions to this group?", false);
    if (grantAll) {
      await handleGrant(group.name, "all");
      return;
    }
...
export async function handleGrantUserInteractive() {
  try {
    ...
    const grantAll = await confirm("Grant all permissions to this user?", false);
    if (grantAll) {
      await handleGrantUser(user.email, "all");
      return;
    }
...
/**
 * User-specific permission management for a pre-selected user.
 * Used from the User Details menu in the main CLI.
 */
export async function handleUserPermissionsInteractiveForUser(user: {
  id: string;
  email: string;
  name: string | null;
}) {
  try {
    while (true) {
      header("User Permissions", "Manage direct permissions for the selected user");

      displayKeyValue("Email", user.email);
      displayKeyValue("Name", user.name || "-");
      separator();

      const action = await menu("Select a permission action:", [
        {
          key: "list",
      ...
      switch (action) {
        case "list":
          ...
          break;
        case "grant":
          await grantPermissionsForUser(user);
          await waitForEnter();
          break;
        case "revoke":
          await revokePermissionsForUser(user);
          await waitForEnter();
          break;
        case "back":
        default:
          notice("Returning to user actions.", "info");
          return;
      }
...
async function grantPermissionsForUser(user: {
  id: string;
  email: string;
  name: string | null;
}) {
  ...
  const grantAll = await confirm(
    `Grant all permissions to user "${user.email}"?`,
    false
  );
...
async function revokePermissionsForUser(user: {
  id: string;
  email: string;
  name: string | null;
}) {
