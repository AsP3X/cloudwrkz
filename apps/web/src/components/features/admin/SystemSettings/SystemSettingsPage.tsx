...
import { purgeDeletedAccounts, updateLinksDefaultPageSize, updateQrLoginRequestsPerMinute } from "@/server/actions/admin/settings";
import {
...
import type { getQrLoginRequestsPerMinute } from "@/server/lib/qr-login-rate-limit";
...
interface SystemSettingsPageProps {
  ...
  qrLoginRequestsPerMinute: Awaited<ReturnType<typeof getQrLoginRequestsPerMinute>>;
}
...
export function SystemSettingsPage({
  ...
  qrLoginRequestsPerMinute: initialQrLoginRequestsPerMinute,
  ...
  const [qrLoginRequestsPerMinute, setQrLoginRequestsPerMinute] = useState(initialQrLoginRequestsPerMinute);
  ...
  const handleQrLoginRequestsPerMinuteSave = async () => {
    ...
    setQrMessage(null);
    try {
      const result = await updateQrLoginRequestsPerMinute(qrLoginRequestsPerMinute);
      if (result.success) {
        setQrMessage({ type: "success", text: "QR code rate limit saved." });
        router.refresh();
      } else {
        setQrMessage({ type: "error", text: result.error ?? "Failed to save." });
      }
    } catch {
      setQrMessage({ type: "error", text: "Failed to save." });
    } finally {
      setQrSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* QR code login: rate limit (admin only) */}
      <div className="bg-white/80 dark:bg-neutral-900/80 backdrop-blur-sm rounded-xl shadow-soft-lg border border-neutral-200/50 dark:border-neutral-800/50 p-6">
        <h2 className="text-xl font-semibold text-neutral-900 dark:text-neutral-100 mb-4">
          QR code login
        </h2>
        <p className="text-sm text-neutral-600 dark:text-neutral-400 mb-4">
          Maximum number of QR login attempts (new QR codes requested) allowed per minute, globally. Helps prevent abuse of the &quot;Sign in with QR code&quot; option.
        </p>
        <div className="flex flex-wrap items-center gap-3">
          <label className="flex items-center gap-2 text-sm text-neutral-700 dark:text-neutral-300">
            ...
            <input
              ...
              onChange={(e) => {
                const v = e.target.value === "" ? MIN_QR_REQUESTS_PER_MINUTE : parseInt(e.target.value, 10);
                if (!Number.isNaN(v)) setQrLoginRequestsPerMinute(Math.max(MIN_QR_REQUESTS_PER_MINUTE, Math.min(MAX_QR_REQUESTS_PER_MINUTE, v)));
              }}
