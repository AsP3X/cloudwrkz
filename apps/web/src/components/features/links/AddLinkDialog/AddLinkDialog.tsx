...
import {
  createLink,
  extractLinkMetadataAction,
  checkDuplicateUrl,
  getLinkTagSuggestions,
  getLink,
...
export function AddLinkDialog({ open, onOpenChange, selectedCollectionId, selectedCollectionName, selectedCollectionColor }: AddLinkDialogProps) {
  ...
  type DuplicateLinkSummary = { id: string; title: string; url: string; createdAt: Date };
    ...
    try {
      ...
      if (result.success) {
        ...
        if (hasDuplicates || hasSimilar) {
          ...
          const ids = result.duplicateLinkIds || result.similarLinkIds || [];
          if (ids.length > 0) {
            try {
              const details = await Promise.all(
                ids.map(async (id) => {
                  ...
                  return {
                    ...
                    url: existing.url,
                    createdAt: new Date((existing as any).createdAt),
                  } as DuplicateLinkSummary;
                })
              );
              setDuplicateLinks(details.filter((d): d is DuplicateLinkSummary => d !== null));
            } catch {
              setDuplicateLinks([]);
            }
          } else {
