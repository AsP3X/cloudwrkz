...
const RatingInput = ({ watch, setValue }: { watch: any; setValue: any }) => {
  const [localHoveredRating, setLocalHoveredRating] = React.useState<number | null>(null);
  const localCurrentRating = watch("rating") ? parseInt(watch("rating")) : null;
        ...
        {[1, 2, 3, 4, 5].map((star) => (
          <button
            ...
            onClick={() => {
              ...
              setValue("rating", newRating ? newRating.toString() : "", { shouldValidate: true });
            }}
...
export const LinkEditForm = ({ link, collections, onCancel, onSaveSuccess, renderRatingField, onRatingInputReady, onFormMethodsReady }: LinkEditFormProps) => {
  ...
  React.useEffect(() => {
    if (onRatingInputReady) {
      const createRatingInput = (formProps: { watch: any; setValue: any }) => {
        return <RatingInput watch={formProps.watch} setValue={formProps.setValue} />;
      };
  ...
  return (
    <FormProvider {...methods}>
      ...
      {/* Tags */}
      <div>
        <label htmlFor="link-edit-tags" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">
          Tags
        </label>
