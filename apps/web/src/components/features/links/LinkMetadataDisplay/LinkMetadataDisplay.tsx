...
export function LinkMetadataDisplay({ metadata }: LinkMetadataDisplayProps) {
  ...
  const formatValue = (key: string, value: any): React.ReactNode => {
    ...
    // GitHub numeric stats (format with locale separators)
    if (
      [
        "githubStars",
        "githubForks",
    ...
    // GitHub topics / branches as badges
    if (
      (key === "githubTopics" || key === "githubBranches") &&
      Array.isArray(value)
    ) {
    ...
    if (key === "githubLastPushedAt" && typeof value === "string") {
      ...
      const formatted =
        isNaN(date.getTime()) || !githubDateFormatter
          ? value
          : githubDateFormatter.format(date);
      return (
        <span className="text-sm text-neutral-700 dark:text-neutral-300 leading-relaxed">
          {formatted}
        </span>
