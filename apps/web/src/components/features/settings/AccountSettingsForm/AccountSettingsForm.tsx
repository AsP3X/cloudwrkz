    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.fieldErrors) {
          // Prefer field-specific messages when available
          const fieldMessages = Object.values(result.fieldErrors).flat();
          if (fieldMessages.length > 0) {
            setPasswordError(fieldMessages.join(", "));
          } else {
      ...
      setPasswordError(
        getServerActionErrorMessage(error, "An unexpected error occurred. Please try again.")
      );
    }
  };
