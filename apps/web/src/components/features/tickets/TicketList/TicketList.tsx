...
interface TicketListProps {
  ...
  /** When true, show selection checkboxes and bulk actions. */
  showBulkSelect?: boolean;
  /** When true, user can edit/archive/delete tickets (agent or equivalent). */
  isAgent?: boolean;
}
...
export const TicketList = ({ tickets, viewMode, showBulkSelect = false, isAgent = false }: TicketListProps) => {
  ...
  const handleSingleArchive = React.useCallback(
    async (ticketId: string) => {
      ...
      setError(null);
      try {
        const result = await bulkArchiveTickets([ticketId]);
        if (result.success) {
          router.refresh();
        } else {
          setError(result.error || "Failed to archive ticket");
        }
      } catch (err) {
        setError(getServerActionErrorMessage(err));
      } finally {
  ...
  const getTicketContextMenuItems = React.useCallback(
    ...
    [isAgent, isProcessing, router, handleSingleArchive, handleOpenDeleteForTicket]
  );
