...
// Module key -> nav item for the "Work" section (user-facing module pages)
const MODULE_NAV_ITEMS: Readonly<Record<string, { name: string; href: string; icon: () => JSX.Element }>> =
  Object.freeze({
...
const getNavSections = (options: GetNavSectionsOptions): ReadonlyArray<NavSection> => {
  const {
    enabledModuleKeys,
    canViewPermissions,
    canViewDbConsole,
  ...
  // Work section: module pages the admin has permission to access
  const workItems = enabledModuleKeys
  ...
  if (workItems.length > 0) {
    sections.push(
      Object.freeze({
        ...
        items: Object.freeze(workItems),
      })
    );
  }

  // User Management: only show section and items when user has the corresponding permission
  ...
  if (userManagementItems.length > 0) {
    sections.push(
      Object.freeze({
        ...
        icon: UsersIcon,
        defaultExpanded: true,
        items: Object.freeze(userManagementItems),
      })
    );
  }
  ...
  // System section: only include items the user has permission for
