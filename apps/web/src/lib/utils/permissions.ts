...
export async function getUserPermissions(userId: string): Promise<Set<string>> {
  ...
  // Roles are purely categorical; they do not grant permissions.
...
export async function hasTicketPermission(
  ...
  // Note: we intentionally do NOT treat admin.tickets.manage as a view-all
  // permission here; global visibility is controlled by tickets.view_all.
  if (action === "view") {
    const hasGeneral = permissions.has("tickets.view") || permissions.has("tickets.view_all");
    if (process.env.NODE_ENV === "development") {
