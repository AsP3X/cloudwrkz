...
/** Schema for a collection entry in exported/imported JSON. */
const importCollectionRefSchema = z.object({
 ...
 * URL is required; other fields optional. Accepts exported JSON shape (nulls, collections).
 */
export const importLinkRowSchema = z.object({
  ...
  title: z
    .union([z.string().max(500).trim(), z.null(), z.undefined()])
    .transform((s) => (typeof s === "string" ? s : "")),
  description: z
    .union([z.string().max(2000).trim(), z.null(), z.undefined()])
    .transform((s) => (typeof s === "string" ? s : "")),
  linkType: linkTypeEnum.nullable().optional().transform((v) => v ?? undefined),
  tags: z.union([
    z.array(z.string().max(100)),
    z.string().transform((s) => (s ? s.split(/[;,]/).map((t) => t.trim()).filter(Boolean) : [])),
    z.null(),
    z.undefined(),
  ]).optional().default([]).transform((v) => (Array.isArray(v) ? v : [])),
  notes: z
    .union([z.string().max(10000).trim(), z.null(), z.undefined()])
    .transform((s) => (typeof s === "string" ? s : "")),
  isFavorite: z.boolean().optional().nullable().transform((v) => v ?? false),
  rating: z.number().int().min(1).max(5).nullable().optional(),
  collections: z.array(importCollectionRefSchema).optional().default([]),
});
