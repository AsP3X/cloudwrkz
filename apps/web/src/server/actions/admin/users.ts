...
export async function getAllUsersAdmin(filters: UserFilters = {}) {
  ...
  await requireAnyPermission("admin.users.view", "admin.users.update", "admin.users.delete", "admin.users.create");
...
export async function getUserByIdAdmin(userId: string) {
  const { requireAnyPermission } = await import("@/lib/utils/auth-server");
  await requireAnyPermission("admin.users.view", "admin.users.update", "admin.users.delete");
...
export async function createUserAdmin(input: z.infer<typeof createUserSchema>): Promise<ActionResult<{ id: string }>> {
  try {
    const { requireAnyPermission } = await import("@/lib/utils/auth-server");
    await requireAnyPermission("admin.users.create");
...
export async function updateUserAdmin(
  ...
  try {
    const { requireAnyPermission } = await import("@/lib/utils/auth-server");
    const admin = await requireAnyPermission("admin.users.update");
...
export async function deleteUserAdmin(userId: string): Promise<ActionResult> {
  try {
    const { requireAnyPermission } = await import("@/lib/utils/auth-server");
    const currentUser = await requireAnyPermission("admin.users.delete");
...
export async function updateUserRoleAdmin(
  ...
  try {
    const { requireAnyPermission } = await import("@/lib/utils/auth-server");
    const currentUser = await requireAnyPermission("admin.users.update");
