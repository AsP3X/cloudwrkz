...
export async function shareCollection(
  ...
  try {
    ...
    auditLog({
      action: "collections.share",
      userId: user.id,
      resourceType: "collection",
      resourceId: collectionId,
      context: { sharedWithUserId: input.userId, role: input.role },
    });
    revalidatePath("/dashboard/links");
...
export async function updateCollectionMember(
  ...
  try {
    ...
    auditLog({
      action: "collections.member.update",
      userId: user.id,
      resourceType: "collection",
      resourceId: collectionId,
      context: { memberUserId: userId, role },
    });
    revalidatePath("/dashboard/links");
    revalidatePath(`/dashboard/links/collections/${collectionId}`);
    return {
...
export async function removeCollectionMember(
  ...
  try {
    ...
    auditLog({
      action: "collections.member.remove",
      userId: user.id,
      resourceType: "collection",
      resourceId: collectionId,
      context: { removedUserId: userId },
    });
    revalidatePath("/dashboard/links");
    revalidatePath(`/dashboard/links/collections/${collectionId}`);
    return {
