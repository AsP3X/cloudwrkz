...
export async function getGroups() {
  ...
  const canManageGroups = permissions.has("admin.groups.manage" as any);

  if (canManageGroups) {
    // Users with admin.groups.manage can see all groups
  ...
  // Other users can only see groups they're members of
...
export async function getGroup(id: string) {
  ...
  // Check if user has access (via permission or membership)
  const { getUserPermissions } = await import("@/lib/utils/permissions");
  const permissions = await getUserPermissions(user.id);
  const canManageGroups = permissions.has("admin.groups.manage" as any);
...
export async function createGroup(input: GroupInput): Promise<ActionResult<{ id: string }>> {
  try {
    await requireAnyPermission("admin.groups.manage");
