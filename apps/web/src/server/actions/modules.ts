...
export async function canUserViewModule(userId: string, moduleKey: ModuleKey): Promise<boolean> {
  ...
  // If user has any link permission (links.view, links.view_all, links.create, etc.), they can see the module
  if (moduleKey === MODULE_KEYS.LINKS) {
    const hasModulePermission = userPermissions.has(permissionKey as any);
    const hasLinkPermission =
      userPermissions.has("links.view" as any) ||
      userPermissions.has("links.view_all" as any) ||
 ...
 * Audited: who (userId), what (module key, enabled state), when (createdAt), device (ipAddress, userAgent).
 */
export async function setModuleEnabled(
  moduleKey: ModuleKey,
  enabled: boolean
) {
  const user = await requireAnyPermission("admin.modules.manage");
  ...
  void writeAuditLog({
    action: "modules.enabled_changed",
    userId: user.id,
    resourceType: "module",
