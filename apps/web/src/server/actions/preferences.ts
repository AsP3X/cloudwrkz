...
export async function getPreferences() {
  ...
  const language = (dbUser?.locale && /^[a-z]{2,5}$/.test(dbUser.locale))
    ? dbUser.locale
    : "en";

  return {
    theme,
    language: language as "en" | "es" | "fr" | "de",
...
export async function updatePreferences(
  ...
  try {
    ...
    // Prepare update data
    const updateData: { timezone?: string; theme?: string; locale?: string } = {};
    ...
    // Update locale/language if provided (server stores concrete code; "system" is stored as-is for app to interpret)
    if (language !== undefined) {
      const validLocale = ["en", "es", "fr", "de", "system"].includes(language) ? language : "en";
      updateData.locale = validLocale;
    }
