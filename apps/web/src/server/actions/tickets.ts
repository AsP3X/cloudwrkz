...
async function getAuditRequestMeta() {
...
export async function createTicket(input: TicketInput): Promise<ActionResult<{ id: string }>> {
  try {
    ...
    const requestMeta = await getAuditRequestMeta();
    auditLog({
...
export async function getTickets(filters?: {
  ...
  dueBefore?: string; // ISO date string — tickets with dueDate before this date
  dueAfter?: string; // ISO date string — tickets with dueDate after this date
  overdueOnly?: boolean; // Only tickets where dueDate is in the past
  ...
  const canViewAllTickets = userPermissions.has("tickets.view_all");
  ...
  // Due date filtering
  if (filters?.dueBefore || filters?.dueAfter || filters?.overdueOnly) {
    baseFilters.dueDate = {};
    if (filters.overdueOnly) {
      baseFilters.dueDate.lt = new Date();
      baseFilters.dueDate.not = null;
    } else {
      if (filters.dueBefore) {
        const endDate = new Date(filters.dueBefore);
        endDate.setHours(23, 59, 59, 999);
        baseFilters.dueDate.lte = endDate;
      }
      if (filters.dueAfter) {
        baseFilters.dueDate.gte = new Date(filters.dueAfter);
      }
    }
  }
...
/** Select shape for getTicket – used for correct return typing including relations. */
const getTicketSelect = {
  id: true,
  ticketNumber: true,
  title: true,
  description: true,
  ...
  comments: {
    where: {} as { isAgentOnly?: boolean },
    select: {
      id: true,
      content: true,
      contentHtml: true,
      contentPlain: true,
      mergedFromTicketNumber: true,
      createdAt: true,
      updatedAt: true,
      isAgentOnly: true,
      userId: true,
...
export type TicketDetail = Prisma.TicketGetPayload<{ select: typeof getTicketSelect }>;
...
export async function getTicket(id: string): Promise<TicketDetail | null> {
  ...
  // Determine if the user can see internal/agent-only comments
  const { getUserPermissions: getUserPermissionsForComments } = await import("@/lib/utils/permissions");
  const userPermissionsForComments = await getUserPermissionsForComments(user.id);
  const canSeeInternalComments = userPermissionsForComments.has("tickets.comments.view_internal");
  ...
  const ticket = await prisma.ticket.findUnique({
    ...
    select: {
      ...getTicketSelect,
      comments: {
        ...getTicketSelect.comments,
        where: !canSeeInternalComments ? { isAgentOnly: false } : {},
      },
    },
  });
  ...
  // Check dynamic ticket permission or general permissions for ALL roles,
  // including admins. Admins are no longer automatically allowed to view
  // every ticket – they must have explicit permissions or other access paths.
  {
    ...
    // Check if user has "view all" style permissions
    const canViewAllTickets = userPermissions.has("tickets.view_all");
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      console.log(`[getTicket] Can view all tickets: ${canViewAllTickets}`);
    ...
    // we require it specifically for other users.
    //
    // Users with "view all" style permissions can always see the ticket.
    ...
    if (isCreator) {
      ...
      // Users with explicit "view all" style permissions can see any ticket
      hasAccess = true;
      if (process.env.NODE_ENV === "development") {
        console.log(`[getTicket] User ${user.id} has view-all permissions - allowing access`);
      }
      ...
      if (process.env.NODE_ENV === "development") {
        console.log(
          `[getTicket] Dynamic permission exists for this ticket - requiring specific permission: ${hasAccess}`
        );
      }
    } else {
      // No dynamic permission exists and user does NOT have view-all permissions.
      // We defer to role/assignment-based checks below (agents) or deny.
      hasAccess = false;
      if (process.env.NODE_ENV === "development") {
        console.log(
          `[getTicket] No dynamic permission and no view-all permission - falling back to role/assignment checks`
        );
      }
    }
    ...
    if (!hasAccess) {
      ...
      if (user.role === "AGENT") {
        ...
        if (ticket.assignedToId === user.id) {
          if (process.env.NODE_ENV === "development") {
            console.log(
              `[getTicket] Agent ${user.id} has access - ticket is assigned to them directly`
            );
          }
          ...
          if (!membership) {
            ...
            if (process.env.NODE_ENV === "development") {
              console.log(
                `[getTicket] Agent ${user.id} is not assigned to ticket and not a member of group ${ticket.assignedToGroupId}, denying access`
              );
            }
            return null;
          }
          ...
          if (process.env.NODE_ENV === "development") {
            console.log(
              `[getTicket] Agent ${user.id} has access - is a member of group ${ticket.assignedToGroupId}`
            );
          }
          hasAccess = true;
        } else {
          ...
          if (process.env.NODE_ENV === "development") {
            console.log(
              `[getTicket] Agent ${user.id} has no access - ticket is not assigned to them and has no group assignment`
            );
          }
          return null;
        }
      } else {
        // No permission and not an agent (including admins without explicit permissions),
        // deny access.
        if (process.env.NODE_ENV === "development") {
          console.log(
            `[getTicket] User ${user.id} (role: ${user.role}) has no permission and no agent access, denying access`
          );
        }
        return null;
      }
    ...
    if (process.env.NODE_ENV === "development") {
      if (hasAccess) {
        console.log(
          `[getTicket] User ${user.id} has access (dynamic: ${hasSpecificDynamicPermission}, viewAll: ${canViewAllTickets}, agent assignment: ${
            user.role === "AGENT" &&
            (ticket.assignedToId === user.id || ticket.assignedToGroupId)
          }), allowing access`
        );
      }
    }
...
/**
 * Lightweight existence check for a ticket ID.
...
/**
 * Merge another ticket into the current ticket.
 * Moves all comments, activities, and timers from the source ticket to the target ticket,
 * and marks the moved records with the original ticket number.
 */
export async function mergeTicketIntoCurrent(
  targetTicketId: string,
  sourceIdentifier: string
): Promise<ActionResult<{ targetTicketId: string; sourceTicketId: string }>> {
  try {
    const user = await requireAuth();

    // Require strong permissions – dedicated merge permission, with fallbacks.
    await requireAnyPermission("tickets.merge", "tickets.update", "admin.tickets.manage");
    ...
    // Resolve source ticket by id first, then by ticketNumber
    let sourceTicket =
      (await prisma.ticket.findUnique({
        where: { id: sourceIdentifier },
        select: {
          id: true,
          ticketNumber: true,
          status: true,
      ...
      (await prisma.ticket.findUnique({
        where: { ticketNumber: sourceIdentifier },
        select: {
          id: true,
          ticketNumber: true,
          status: true,
    ...
    if (!sourceTicket) {
      return {
        success: false,
        error: "Source ticket not found. Please check the ticket ID or number.",
      };
    }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // Optionally close the source ticket if it's not already closed or cancelled
      if (sourceTicket!.status !== "CLOSED" && sourceTicket!.status !== "CANCELLED") {
    ...
    // Build basic details for merge summary
    const sourceDescription =
      (sourceTicket.descriptionPlain as string | null | undefined) ??
      (sourceTicket.description as string | null | undefined) ??
      null;
    const sourceCreatedByDisplay = formatUserName(
      sourceTicket.createdBy,
      sourceTicket.createdByName
    );

    // Log activity on target ticket (summary of merged ticket)
    await logTicketActivity(
      targetTicket.id,
      "MERGED_FROM_TICKET",
      user.id,
      ...
      sourceTicket.ticketNumber,
      {
        ...
        sourceTitle: sourceTicket.title,
        sourceCreatedBy: sourceCreatedByDisplay,
        sourceDescription,
      }
    );

    // Add a comment on the source ticket to indicate where it was merged
    try {
      const mergedIntoHtml = `<p>Merged into <a href="/dashboard/tickets/${targetTicket.id}">${targetTicket.ticketNumber}</a></p>`;
      await prisma.ticketComment.create({
        data: {
          ticketId: sourceTicket.id,
          userId: user.id,
          content: `Merged into ${targetTicket.ticketNumber}`,
          contentHtml: mergedIntoHtml,
          contentPlain: `Merged into ${targetTicket.ticketNumber}`,
          isAgentOnly: false,
        },
      });
      // Log comment activity on source ticket
      await logTicketActivity(
        sourceTicket.id,
        "COMMENT_ADDED",
        user.id,
        ...
        null,
        null,
        { mergedIntoTicketId: targetTicket.id, mergedIntoTicketNumber: targetTicket.ticketNumber }
      );
    } catch (error) {
      console.error("Failed to add merge comment to source ticket:", error);
    }
    ...
    revalidatePath(`/dashboard/tickets/${targetTicket.id}`);
    revalidatePath(`/dashboard/tickets/${sourceTicket.id}`);
    revalidatePath("/dashboard/tickets");
    ...
    return {
      ...
      message: `Ticket ${sourceTicket.ticketNumber} was merged into ${targetTicket.ticketNumber}`,
    };
  } catch (error) {
    console.error("Merge ticket error:", error);
    return {
      success: false,
      error: "Failed to merge tickets. Please try again.",
    };
  }
}

/**
 * Search tickets for merge suggestions, limited to tickets visible to the current user.
 */
export async function searchTicketsForMerge(
  query: string,
  currentTicketId: string
): Promise<ActionResult<Array<{ id: string; ticketNumber: string; title: string }>>> {
  try {
    const user = await requireAuth();
    ...
    // Reuse existing permission-aware ticket fetching
    ...
    const matches = tickets
      ...
      .filter((ticket) => {
        ...
        const description =
          ((ticket as any).descriptionPlain as string | null | undefined)?.toLowerCase() ||
          ((ticket as any).description as string | null | undefined)?.toLowerCase() ||
          "";
    ...
    console.error("searchTicketsForMerge error:", error);
    return {
      success: false,
      error: "Failed to search tickets. Please try again.",
    };
  }
}
...
export async function updateTicket(
  ...
  try {
    ...
    // Entry-level permission gate: user must be allowed to update/manage tickets.
    ...
    // No implicit role-based bypass – rely on permission model + creator exception.
    // Get ticket type prefix
    const ticketPrefix = getTicketTypePrefix(currentTicket.type);
    // Check dynamic ticket permission
    let canUpdate = await hasTicketPermission(user.id, currentTicket.id, ticketPrefix, "update");
    
    // Fallback: ticket creator can always update their own tickets
    if (!canUpdate) {
      canUpdate = currentTicket.createdById === user.id;
    }
    ...
    // Emit notifications (fire-and-forget, non-blocking)
    const ticketUrl = `/dashboard/tickets/${id}`;
    const ticketRef = currentTicket.ticketNumber || id;

    // Notify assignee when ticket is assigned to them
    if (updateData.assignedToId && updateData.assignedToId !== currentTicket.assignedToId && updateData.assignedToId !== user.id) {
      void createNotification(
        updateData.assignedToId,
        "TICKET_ASSIGNED",
        `You were assigned to ${ticketRef}`,
        currentTicket.title,
        "ticket",
        id,
        ticketUrl
    ...
    // Notify ticket creator when status changes (but not if they made the change themselves)
    if (updateData.status && currentTicket.createdById && currentTicket.createdById !== user.id) {
      void createNotification(
        currentTicket.createdById,
        "TICKET_STATUS_CHANGED",
        `${ticketRef} status changed to ${String(updateData.status).replace(/_/g, " ")}`,
        currentTicket.title,
        "ticket",
        id,
        ticketUrl
...
export async function deleteTicket(id: string): Promise<ActionResult> {
  try {
    ...
    // Entry-level permission gate: user must be allowed to delete/manage tickets.
    ...
    // Check dynamic ticket permission or general permissions.
    // No implicit role-based bypass – rely on permission model + limited resource-based rules.
    // Get ticket type prefix
    const ticketPrefix = getTicketTypePrefix(ticket.type);
    // Check dynamic ticket permission
    let canDelete = await hasTicketPermission(user.id, ticket.id, ticketPrefix, "delete");
    
    // Fallback: creator or assigned agent can delete
    if (!canDelete) {
      canDelete =
...
/**
 * Update a ticket with an explicit userId (for Bearer-token API handlers, e.g. iOS app).
 * Supports partial update; for API only archivedAt is typically sent.
 */
export async function updateTicketWithUserId(
  userId: string,
  id: string,
  input: TicketUpdateInput
): Promise<ActionResult> {
  try {
    const canUpdate = await hasAnyPermission(userId, ["tickets.update", "admin.tickets.manage"]);
    if (!canUpdate) {
      return { success: false, error: "You do not have permission to update tickets" };
    }
    ...
    const ticketPrefix = getTicketTypePrefix(currentTicket.type);
    let canUpdateTicket = await hasTicketPermission(userId, currentTicket.id, ticketPrefix, "update");
    if (!canUpdateTicket) {
      canUpdateTicket = currentTicket.createdById === userId;
    }
    if (!canUpdateTicket) {
      return { success: false, error: "You don't have permission to update this ticket" };
    }
    ...
    console.error("Update ticket (with userId) error:", error);
    return { success: false, error: "Failed to update ticket. Please try again." };
  }
}

/**
 * Delete a ticket with an explicit userId (for Bearer-token API handlers, e.g. iOS app).
 */
export async function deleteTicketWithUserId(userId: string, id: string): Promise<ActionResult> {
  try {
    const user = await prisma.user.findUnique({
      where: { id: userId },
    ...
    const ticketPrefix = getTicketTypePrefix(ticket.type);
    // Allow delete if: creator, assigned agent, has tickets.delete (or type permission), or admin
    let canDeleteTicket =
      ticket.createdById === userId ||
      (user.role === "AGENT" && ticket.assignedToId === userId);
    if (!canDeleteTicket) {
      canDeleteTicket =
        (await hasTicketPermission(userId, ticket.id, ticketPrefix, "delete")) ||
        (await hasAnyPermission(userId, ["admin.tickets.manage"]));
    }
    if (!canDeleteTicket) {
    ...
    console.error("Delete ticket (with userId) error:", error);
    return { success: false, error: "Failed to delete ticket. Please try again." };
  }
}

/**
 * Delete a ticket from form submission (stable Server Action for use in forms).
 * Reads ticketId from formData and redirects to /dashboard/tickets on success.
 */
export async function deleteTicketFromForm(formData: FormData): Promise<void> {
...
export async function addTicketComment(
  ...
  try {
    ...
    // Only users with the dedicated permission can create agent-only comments
    if (isAgentOnly) {
      try {
        await requireAnyPermission("tickets.comments.agent_only");
      } catch {
        return {
          success: false,
          error: "You don't have permission to create internal comments",
        };
      }
    }
    ...
    // Check dynamic ticket permission or general permissions.
    // No implicit role-based bypass – rely on permission model.
    ...
    auditLog({
      ...
      context: { ticketId, isAgentOnly },
      ...requestMeta,
    });

    // Notify ticket assignee and creator of new comment (non-blocking)
    const ticketForNotif = await prisma.ticket.findUnique({
      where: { id: ticketId },
      select: { assignedToId: true, createdById: true, ticketNumber: true, title: true },
    });
    if (ticketForNotif && !isAgentOnly) {
      const ticketRef = ticketForNotif.ticketNumber || ticketId;
      const ticketUrl = `/dashboard/tickets/${ticketId}`;
      const notifyUserIds = new Set<string>();
      if (ticketForNotif.assignedToId && ticketForNotif.assignedToId !== user.id) {
        notifyUserIds.add(ticketForNotif.assignedToId);
      }
      if (ticketForNotif.createdById && ticketForNotif.createdById !== user.id) {
        notifyUserIds.add(ticketForNotif.createdById);
      }
      for (const uid of notifyUserIds) {
        void createNotification(
          uid,
          "TICKET_COMMENT_ADDED",
          `New comment on ${ticketRef}`,
          ticketForNotif.title,
          "ticket",
          ticketId,
          ticketUrl
        );
...
export async function bulkUpdateTickets(
  ...
  try {
    ...
    // Entry-level permission gate: user must be allowed to perform bulk or general updates.
    await requireAnyPermission("tickets.bulk_update", "tickets.update", "admin.tickets.manage");
    ...
    // Use dynamic/global ticket permissions with a fallback for creators.
    ...
    auditLog({
      ...
      resourceType: "ticket",
      resourceId: null,
      context: { ticketIds, count: result.count, updates },
      ...requestMeta,
    });
...
export async function bulkDeleteTickets(
  ...
  try {
    ...
    // Entry-level permission gate: user must be allowed to perform bulk or general deletes.
    await requireAnyPermission("tickets.bulk_delete", "tickets.delete", "admin.tickets.manage");
    ...
    // Use dynamic/global ticket permissions with a fallback for creator/assigned agent.
    ...
    auditLog({
      ...
      resourceType: "ticket",
      resourceId: null,
      context: { ticketIds, count: result.count },
      ...requestMeta,
    });
...
export async function bulkArchiveTickets(
  ...
  try {
    ...
    // Entry-level permission gate: user must be allowed to archive tickets (bulk or single).
    await requireAnyPermission("tickets.bulk_archive", "tickets.archive", "tickets.update", "admin.tickets.manage");

    // Check permissions for all tickets (same model as bulkUpdateTickets)
    const tickets = await prisma.ticket.findMany({
    ...
    for (const t of tickets) {
      const ticketPrefix = getTicketTypePrefix(t.type);
      const hasPermission = await hasTicketPermission(
        user.id,
        t.id,
        ticketPrefix,
      ...
      const canUpdateThis = hasPermission || t.createdById === user.id;
...
export async function bulkUnarchiveTickets(
  ...
  try {
    ...
    // Entry-level permission gate: user must be allowed to unarchive tickets (bulk or single).
    await requireAnyPermission("tickets.bulk_unarchive", "tickets.archive", "tickets.update", "admin.tickets.manage");
