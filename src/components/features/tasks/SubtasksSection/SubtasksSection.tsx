...
export const SubtasksSection = ({ parentTaskId, subtasks, canManage, viewMode: externalViewMode, onViewChange: externalOnViewChange, userTimezone = "UTC" }: SubtasksSectionProps) => {
  ...
  const activeSubtasks = subtasks.filter((task) => task.status !== "COMPLETED");
  const completedSubtasks = subtasks.filter((task) => task.status === "COMPLETED");
  const hasSubtasks = subtasks.length > 0;

  const renderSubtaskCard = (task: Subtask) => (
  ...
  const renderSubtaskMobileCard = (task: Subtask) => (
  ...
  const renderSubtaskRow = (task: Subtask) => (
